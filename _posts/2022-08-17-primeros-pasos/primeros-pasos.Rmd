...
description: |
  Empezar a trabajar en R puede ser abrumador, por esa razón veremos las principales herramientas del `tidyverse`, las cuáles nos serán útil al momento de limpiar una base de dato o transformar aquellas variables con las que contamos. Además, revisaremos los principios del `ggplot2` para armar gráficos.
author:
  - name: Soledad Araya
...
La función `pivot_longer` necesita que definamos las variables que hay que poner en formato "long". En este caso, todas las variables relacionadas con el ranto etario empezaban con "x", por lo que usamos `start_with()` para seleccionarlas. Luego, nombramos la variable que contendrá los nombres de las columnas, y luego el nombre de la variable que tendrá los valores de las columnas.
...
Con esto, será más fácil usar `ggplot2`.
